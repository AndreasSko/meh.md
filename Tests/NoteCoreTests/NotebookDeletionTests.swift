...
final class NotebookDeletionTests: XCTestCase {
    ...
    func testCleanupDiscardsCorruptOwnedImportTemporaryFiles() throws {
        let root = temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        let recoveryDirectory = root.appending(path: "import-recovery")
        try FileManager.default.createDirectory(
            at: recoveryDirectory,
            withIntermediateDirectories: true
        )
        let temporaryURLs = [
            root.appending(path: ".pending-import-interrupted.tmp"),
            recoveryDirectory.appending(
                path: ".scrubbed-import-interrupted.tmp"
            ),
        ]
        ...
        try NotebookImportStorage(directory: root).scrub(
            deletedIDs: [UUID()],
            notebookID: UUID()
        )
    ...
    func testCleanupRetainsCorruptDurableImportJournals() throws {
        for retainedRecovery in [false, true] {
            let root = temporaryDirectory()
            defer { try? FileManager.default.removeItem(at: root) }
            ...
            if retainedRecovery {
                ...
                journalURL = recoveryDirectory.appending(
                    path: "pending-import-retained.json"
                )
            } else {
                try FileManager.default.createDirectory(
                    at: root,
                ...
                journalURL = storage.journalURL
            }
            let corruptBytes = Data("damaged durable import".utf8)
            try corruptBytes.write(to: journalURL)
            ...
            do {
                ...
                XCTFail("Expected strict durable journal validation")
            } catch NotebookDeletionStorageError.invalidImportJournal(let url) {
                XCTAssertEqual(
                    url.resolvingSymlinksInPath(),
                    journalURL.resolvingSymlinksInPath()
                )
            } catch {
